...
function PesticideInventory() {
    ...
    const fetchPesticides = async () => {
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/pesticide-inventory/${email}`);
    ...
    const addPesticide = async (e) => {
        ...
        const pesticideWithUserEmail = { ...newPesticide, user_email: email };
        try {
            const response = await fetch("http://localhost:5000/api/pesticide-inventory/add", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(pesticideWithUserEmail),
            });
