    ...
    return (
        <>
                        ...
                        {vehicles.map((vehicle) => (
                            <tr key={vehicle._id} className="bg-gray-900 hover:bg-gray-800">
                                ...
                                <td className="px-4 py-2 border border-gray-700 space-x-2">
                                    <button onClick={() => updateVehicle(vehicle)}
            ...
            {updateModalOpen && selectedVehicle && (
                ...
                >
                    <div className="bg-gray-800 p-6 rounded-lg shadow-lg w-96">
                        <h2 className="text-lg font-semibold text-gray-200 mb-2">Update Vehicle Running Hours</h2>
                        <form onSubmit={handleUpdateVehicle} className="flex flex-col gap-4">
                            <input
                                type="number"
                                name="curr_running_hours"
                                placeholder="Running Hours"
                                value={selectedVehicle.curr_running_hours || ''}
                                onChange={handleUpdateChange}
                                className="p-2 rounded bg-gray-700 text-white"
                                required
                            />
                            <button type="submit" className="bg-blue-600 hover:bg-blue-500 px-3 py-2 rounded text-white">Update Running Hours</button>
                        </form>
                        <button onClick={() => setUpdateModalOpen(false)} className="text-red-600 hover:text-red-500 mt-2">Close</button>
                    </div>
                </motion.div>
            )}
